...
def _visible(posts, user):
    ...
    if user.is_authenticated:
        base = posts.filter(Q(author=user)|Q(is_published=True, pub_date__lte=timezone.now()))
    return base

def index(request):
    posts = _visible(Post.objects.annotate(comment_count=Count('comments')), request.user)
    page_obj = Paginator(posts, POSTS_PER_PAGE).get_page(request.GET.get('page'))
    return render(request, 'blog/index.html', {'page_obj': page_obj})

def category_posts(request, slug):
    category = get_object_or_404(Category, slug=slug, is_published=True)
    posts = _visible(category.posts.annotate(comment_count=Count('comments')), request.user)
    page_obj = Paginator(posts, POSTS_PER_PAGE).get_page(request.GET.get('page'))
    return render(request, 'blog/category.html', {'category': category, 'page_obj': page_obj})

def post_detail(request, post_id):
    post = get_object_or_404(Post.objects.annotate(comment_count=Count('comments')), pk=post_id)
